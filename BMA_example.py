...
# create event tree
data = pd.read_csv("datasets_test3/Titanic.csv")
show_event_tree(data)

# %%

# create stage tree
st = staged.StagedTree(data,struct_missing_label= "nan_3")
...
for h in range(len(staging_output)):
    if len(model_weights[h])>1:
        ...
        data = model_weights[h].flatten()  # Flatten the array to 1D

        plt.pie(data, labels=labels)#,autopct = lambda p:round(p+0.07)/100,startangle=0,radius =1.25)
...
if len(full_staging_output[h])>1:
...
plt.ylabel('Model weights')
plt.xticks(rotation=90)
# plt.savefig('BMA_full_falls_weights.pdf', format='pdf', dpi=1000)
plt.show()
#save figure as pdf
